...
local ns = "dunedaq.cpuburner.cpuburner";
...
local types = {
    ...
    // TO cpuburner DEVELOPERS: PLEASE DELETE THIS FOLLOWING COMMENT AFTER READING IT
    // The following code is an example of a configuration record
    // written in jsonnet. In the real world it would be written so as
    // to allow the relevant members of CPUBurner to be configured by
    // Run Control
